...
class CPFM_Feedback_Notice {
    ...
    public function cpfm_enqueue_assets() {
        ...
        wp_enqueue_style('cpfm-common-review-style', MFE_PLUGIN_URL . 'admin/feedback/css/cpfm-admin-feedback.css', [], MFE_VERSION);


        wp_enqueue_script(
            'cpfm-common-review-script', 
            MFE_PLUGIN_URL . 'admin/feedback/js/cpfm-admin-feedback.js', 
            ['jquery'], 
            MFE_VERSION,
            true
    ...
    public function cpfm_render_notice_panel() {
        ...
        $output .= '<div class="notice-panel-header">' . esc_html__('Help Improve Plugins', 'mask-form-elementor') . ' <span class="dashicons dashicons-no" id="cpfm_remove_notice"></span></div>';
        ...
        foreach (self::$registered_notices as $key => $notice) {
            ...
            $output .= '<p>' . esc_html($notice['message']) . '<a href="#" class="cpf-toggle-extra">' . esc_html__(' More info', 'mask-form-elementor') . '</a></p>';
            $output .= '</div>';
            
            $output .= '<div class="cpf-extra-info">';
            $output .= '<p>' . esc_html__('Opt in to receive email updates about security improvements, new features, helpful tutorials, and occasional special offers. We\'ll collect:', 'mask-form-elementor') . '</p>';
            $output .= '<ul>';
            $output .= '<li>' . esc_html__('Your website home URL and WordPress admin email.', 'mask-form-elementor') . '</li>';
            $output .= '<li>' . esc_html__('To check plugin compatibility, we will collect the following: list of active plugins and themes, server type, MySQL version, WordPress version, memory limit, site language and database prefix.', 'mask-form-elementor') . '</li>';
            $output .= '</ul>';
            
            $output .= '</div>';
            
            $output .= '<div class="notice-actions">';
            $output .= '<button class="button button-primary opt-in-yes" data-category="' . esc_attr($key) . '" id="yes-share-data" value="yes">' . esc_html__("Yes, I Agree", 'mask-form-elementor') . '</button>';
            $output .= '<button class="button opt-in-no" data-category="' . esc_attr($key) . '" id="no-share-data" value="no">' . esc_html__('No, Thanks', 'mask-form-elementor') . '</button>';
            $output .= '</div>';
